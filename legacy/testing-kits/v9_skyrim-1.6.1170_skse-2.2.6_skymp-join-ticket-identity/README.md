...
- RaceMenu loading overlay and stronger preview stripping + ragged outfit enforcement.
- Join-ticket auth flow (website-issued Ed25519 ticket) and server-side identity allocation (SkyV-Identity).

Changes vs v8:

- Client opens `https://vokunrp.com/game/join` to obtain a join ticket before connecting.
- Server verifies the ticket and allocates a stable numeric profile id via SkyV-Identity.

Logo:

- This kit includes the Vokun logo as:
  - `...\Skyrim Special Edition\Data\Platform\Plugins\skyv-loading-logo.svg`
- You can replace it with either:
  - `skyv-loading-logo.svg` or
  - `skyv-loading-logo.png`
...
7) Join flow (new in v9).

- On the main menu, use the SkyV join UI.
- When you select or create a character, an in-game browser opens:
  - `https://vokunrp.com/game/join`
- Log in with Discord (must be whitelisted).
- The website sends a join ticket to the client, and the client connects.
